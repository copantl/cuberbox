...
-- nexus_router.lua
-- NEXUS Core - Telemetry Bridge

local uuid = session:getVariable("uuid")
local dest = session:getVariable("destination_number")
local caller = session:getVariable("caller_id_number")
local trace_id = session:getVariable("nexus_trace_id")

freeswitch.consoleLog("info", "[NEXUS LUA] Processing call: " .. uuid .. " -> " .. dest .. "\n")

-- Disparar evento personalizado para el Backend en Go
local event = freeswitch.Event("CUSTOM", "nexus::telemetry")
event:addHeader("Nexus-Action", "CALL_ROUTING")
event:addHeader("Nexus-Trace-ID", trace_id)
...
session:setVariable("nexus_status", "ROUTED")
