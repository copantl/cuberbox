...
# =============================================================================
# CUBERBOX NEXUS CORE - COMPILED FREESWITCH CONFIG DEPLOYER V1.0
...
echo -e "${BLUE}=== CUBERBOX Nexus Core Config Deployer (Compiled FS) ===${NC}"
...
DIALPLAN_DEST="$FS_BASE/conf/dialplan/default/01_nexus_core.xml"
...
LUA_DEST="$FS_BASE/scripts/nexus_router.lua"
cp setup/nexus_router.lua "$LUA_DEST"
...
mkdir -p /opt/nexus-core/recordings
chown -R $(ls -ld "$FS_BASE/conf" | awk '{print $3":"$4}') /opt/nexus-core/recordings
echo -e "${GREEN}[OK] Directorio /opt/nexus-core/recordings listo.${NC}"
